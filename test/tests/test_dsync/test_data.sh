...
DFILEMAKER_TEST_BIN=${DFILEMAKER_TEST_BIN:-${5}}

echo "Using dsync binary at: $DSYNC_TEST_BIN"
echo "Using src directory at: $DSYNC_SRC_BASE"
echo "Using dest directory at: $DSYNC_DEST_BASE"
echo "Using directory tree: $DSYNC_TREE_NAME"
echo "Using dfilemaker binary at: $DFILEMAKER_TEST_BIN"
...
# args expected by dfilemaker1 (does not create trees, files all identical data)
#$DFILEMAKER_TEST_BIN 100 4MB

# args expected by dfilemaker (creates trees, files all different data)
$DFILEMAKER_TEST_BIN 5000 6 25
popd
